...
def list_bookings(global_booking_list, global_table_allocated: list) -> None:
    ...
    if len(temp) == 0:
        ...
        for x in temp:
            print(
                f"{x[0]}, {x[1]}, {x[2]} (Ticket {x[4]}), {x[3]}, "
                f"Allocated table: {_table_allocated_for(x[2], x[4], global_table_allocated)}."
            )


def _table_allocated_for(
    date, ticket_code: str, global_table_allocated: list
) -> int | None:
    """
    ...
    # on the date we want.
    table_allocated = [
        x for x in global_table_allocated if x[0] == date and x[1] == ticket_code
    ]
...
def _allocate_table(
    global_table_allocated: list,
    table_file_list: list,
    date: str,
    ticket_code: str,
    table_number: str,
) -> int:
...
def allocate_table(
    ...
    """
    table_allocate_status = _allocate_table(
        global_table_allocated, table_file_list, date, ticket_code, table_number
    )
